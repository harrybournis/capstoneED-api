...
RSpec.describe V1::ProjectEvaluationsController, type: :controller do
  ...
  context 'Student' do
    ...
    # 	pe = FactoryBot.create(:project_evaluation, user: @student, percent_complete: 89)
    ...
    it 'GET index_with_project returns project_health, team_health', { docs?: true } do
      FactoryBot.create(:project_evaluation, user: @student, project_id: @project.id, iteration_id: @project.iterations[0].id)
  ...
  context 'Lecturer' do
    ...
    it 'POST create creates new project_evaluation if lecturer is in project' do
      attr = FactoryBot.attributes_for(:project_evaluation).merge(user_id: @lecturer.id, project_id: @project.id, iteration_id: @project.iterations[0].id, feelings: valid_feelings_params)
    ...
    it 'POST create returns 422 unprocessable_entity if lecturer does not belong in project' do
      now = DateTime.now
      assignment = FactoryBot.create(:assignment, start_date: now, end_date: now + 1.month)
      project = FactoryBot.create(:project, assignment: assignment)
      iteration = FactoryBot.create(:iteration, assignment: assignment, start_date: now + 1.minute, deadline: now + 28.days)

      attr = FactoryBot.attributes_for(:project_evaluation).merge(user_id: @lecturer.id, iteration_id: iteration.id, project_id: project.id)
      post :create, params: attr
    ...
    it 'POST create returns 422 unprocessable_entity if iteration is not active' do
      Timecop.travel(DateTime.now + 2.months) do
        ...
        request.cookies['access-token'] = mock_request.cookies['access-token']
        request.headers['X-XSRF-TOKEN'] = mock_request.headers['X-XSRF-TOKEN']
        attr = FactoryBot.attributes_for(:project_evaluation).merge(user_id: @lecturer.id, project_id: @project.id, iteration_id: @project.iterations[0].id)
        post :create, params: attr
    ...
    # 	pe = FactoryBot.create(:project_evaluation_lecturer, user: @lecturer, percent_complete: 89)
    # 	patch :update, params: { id: pe.id, project_id: pe.project_id, percent_complete: pe.percent_complete }
