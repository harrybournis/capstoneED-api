...
RSpec.describe Extension, type: :model do
		...
		describe 'validations' do
			...
			it 'validates uniqueness of iteration_id with scope team_id' do
				...
				ext2 = FactoryGirl.build(:extension, team_id: ext1.team_id, deliverable_id: ext1.deliverable_id)
				expect(ext2.valid?).to be_falsy
				expect(ext2.errors[:deliverable_id][0]).to eq('already exists for this team_id')
			end
		end
