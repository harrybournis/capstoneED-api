...
RSpec.describe PaForm, type: :model do
	...
	describe 'Questions' do
		...
		it 'must be an array that is not empty' do
			# empty array
		...
		it 'can not have empty text for a question' do
	...
	describe 'Dates' do
		...
		it 'validates that deadline with extension adds the time of the extension' do
			start = 1.day.to_i
			finish = 4.days.to_i
			pa_form = create(:pa_form, start_offset: start, end_offset: finish)
			project = create(:project)
			extension  = create(:extension, deliverable_id: pa_form.id, project_id: project.id)
		...
		it 'return only deadline if iteration does not exist' do
			...
			pa_form = create(:pa_form, start_offset: start, end_offset: finish)
			project = create(:project)
			wrong_pa = create(:pa_form)
			extension  = create(:extension, deliverable_id: wrong_pa.id, project_id: project.id)
	...
	it '.active returns only the forms that can be currently submitted' do
		now = DateTime.now
		iteration1 = create(:iteration, start_date: now + 3.days, deadline: now + 5.days)
		iteration2 = create(:iteration, start_date: now + 4.days, deadline: now + 6.days)
		create(:pa_form, iteration: iteration1, start_offset: 2.days.to_i, end_offset: 5.day.to_i)
		create(:pa_form, iteration: iteration2, start_offset: 2.days.to_i, end_offset: 6.day.to_i)
		expect(PaForm.all.length).to eq 2

		# no form is active because the start offset is 2 days after the deadline
		...
		Timecop.travel(now + 6.days + 6.days + 1.minute) do
			expect(PaForm.active.length).to eq 0
		end
	end
	...
	it '#active? returns true if current time is after start_date and before deadline' do
		...
		Timecop.travel iteration.deadline + 5.minutes do
			expect(pa.active?).to be_truthy
		end

		Timecop.travel iteration.deadline + 5.days do
