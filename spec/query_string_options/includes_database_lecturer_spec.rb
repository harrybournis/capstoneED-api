		...
		describe 'Projects' do
			...
			context 'Invalid' do
				it 'should render errors for invalid includes params' do
					...
					expect(body['errors']['base'].first).to eq("Invalid 'includes' parameter. Project resource for Lecturer user accepts only: lecturer, unit, teams, students. Received: teams,unit,lecturer,banana.")

					expect {
						get :show, params: { id: @project.id, includes: 'teams,unit,lecturer,banana,manyother,wrong_params,$@^&withsymbols,*,**' }
					}.to make_database_queries(count: 0)
					expect(status).to eq(400) #1
					expect(body['errors']['base'].first).to eq("Invalid 'includes' parameter. Project resource for Lecturer user accepts only: lecturer, unit, teams, students. Received: teams,unit,lecturer,banana,manyother,wrong_params,$@^&withsymbols,*,**.")

					expect {
						get :show, params: { id: @project.id, includes: '*,**' }
					}.to make_database_queries(count: 0)
					expect(status).to eq(400) #1
					expect(body['errors']['base'].first).to eq("Invalid 'includes' parameter. Project resource for Lecturer user accepts only: lecturer, unit, teams, students. Received: *,**.")
				end
			end
		end
