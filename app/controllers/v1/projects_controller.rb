...
class V1::ProjectsController < ApplicationController
  ...
  before_action -> { allow_if_lecturer(index_with_assignment_error_message) },
                only: :index_with_assignment
  before_action -> { allow_if_lecturer(index_with_unit_error_message) },
                only: :index_with_unit
  # before_action -> { allow_if_student(index_error_message) },
  #               only: :index
  ...
  def index
    if current_user.student?
      serialize_collection current_user.projects(includes: includes_array).active,
                           :ok,
  ...
  def index_with_assignment
    @projects = current_user.projects(includes: includes_array)
  ...
  def index_with_unit
    @projects = current_user.projects(includes: includes_array)
                            .where(['projects.unit_id = ?', params[:unit_id]])
    serialize_collection @projects, :ok
  end
  ...
  def update
    if project_update_params.empty?
      render json: format_errors({ base: ["none of the given parameters can be updated by the current user"] }),
             status: :bad_request
      return
    end
  ...
  def project_params
    params.permit(:project_name, :team_name, :description,
                  :logo, :enrollment_key, :student_id, :assignment_id)
  end
