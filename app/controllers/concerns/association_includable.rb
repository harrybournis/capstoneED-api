...
module AssociationIncludable
  ...
  # object, depending on the user type.
  #
  # @param  [Array]   associations    The <resource>_associations method
  #                                   that conrresponds to the current resource.
  #                                   e.g. project_associations
  # @param  [Array]   includes_array  An array of strings. The includes param
  #                                   can be transformed using the includes_arry
  #                                   method.
  # @param  [String]  resource        A String representing the name of the
  #                                   resource. Only used to display errors to
  #                                   the user.
  # @return [Boolean]
  def validate_includes # (associations, includes_array, resource)
    ...
    render json: format_errors({ base: ["Invalid 'includes' parameter. #{resource} resource for #{current_user.type} user accepts only: #{associations.join(', ')}. Received: #{params[:includes]}."] }),
           status: :bad_request
    false
  end
