class Docs::V1::Students < ApplicationController
	...
	description <<-EOS
		Register a new Student resource that is authenticatable with email and password.
	...
	error code: 403, desc: 'The User logged in is not of type Student'
	error code: 422, desc: "Failed to save User. Params are invalid. See errors in response body."
	...
	description <<-EOS
		Update an existing Student. If the request aims to update the user's password,
		along with <b>password</b> and <b>password_confirmation</b>, the user's current password must
		be provided as <b>current_password</b> in the params. A Student can only delete themselves.
	EOS
	def update
	end
	...
	example DocHelper.format_example(status = 422, nil, body = "{\n  \"errors\": {\n    \"current_password\": [\n      \"is invalid\"\n    ]\n  }\n}")
	description <<-EOS
		Delete a Student. the user's id must be provided as part of the url, and their valid password must
		be included in the body as <b>current_password </b>. A student can only delete themselves.
	EOS
	def destroy
	end
end
